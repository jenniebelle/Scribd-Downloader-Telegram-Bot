...
const limiter = rateLimit({
  ...
  keyGenerator: (req) => req.from?.id?.toString() || 'unknown',
});
    ...
    for (const user of users) {
      queue.add(() =>
        bot.sendMessage(user._id, promoMessage, {
          ...
          console.error(`❌ Failed to message ${user._id} (${user.username}):`, err.message);
        })
      );
    }
  } catch (error) {
...
bot.on('message', async (msg) => {
  ...
  try {
    ...
    if (
      (msg.new_chat_member?.id || msg.left_chat_member?.id) ||
      (msg && ['left', 'kicked'].includes(joinStatus1) || ['left', 'kicked'].includes(joinStatus2))
    ) {
      await bot.deleteMessage(chatId, messageId);
      await bot.sendMessage(
        fromId,
        `Welcome ${name} 🔓 🔰 | You must subscribe to the channels to use the bot for free.`,
        {
